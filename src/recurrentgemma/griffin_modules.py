...
class ModifiedRecurrentBlock(nn.Module):
    ...
    def input_to_state(
            self,
            input_bxtxd: at.Activations,
            segment_pos_bxt: at.SegmentPos,
            previous_state_cache: RecurrentBlockCache | None = None,
            return_cache: bool = True,
    ) -> tuple[Tensor, Tensor, RecurrentBlockCache, dict[str, Any]]:

        seq_len = input_bxtxd.shape[1]

        # y branch.
        y = self.linear_y(input_bxtxd)
        y_linear_bxtxh = y
        y = gelu(y)

        # x branch.
        x = self.linear_x(input_bxtxd)
        x_linear_bxtxh = x

        if previous_state_cache.conv1d_state is None and seq_len == 1:
            conv1d_cache_input_bxtemporalxh = self._pad_input_for_conv1d(x)
            previous_state_cache = RecurrentBlockCache(conv1d_state=conv1d_cache_input_bxtemporalxh,
                                                       rg_lru_state=previous_state_cache.rg_lru_state)

        x, conv1d_state_bxdxh = self.conv_1d(
            x=x,
            segment_pos=segment_pos_bxt,
            cache=None if previous_state_cache is None else previous_state_cache.conv1d_state,
            return_cache=return_cache,
        )
        x_conv_bxtxh = x
        x, lru_state_1xbxh, lru_internals = self.rg_lru(
            x=x,
            segment_pos=segment_pos_bxt,
            cache=None if previous_state_cache is None else previous_state_cache.rg_lru_state,
            return_cache=return_cache,
            return_internals=True,
        )
        x_lru_bxtxh = x

        recurrent_block_internals = {
            "y_linear_bxtxh": y_linear_bxtxh,
            "y_bxtxh": y,
            "x_linear_bxtxh": x_linear_bxtxh,
            "x_conv_bxtxh": x_conv_bxtxh,
            "x_lru_bxtxh": x_lru_bxtxh,
            "lru_internals": lru_internals,
        }
        new_cache = RecurrentBlockCache(rg_lru_state=lru_state_1xbxh, conv1d_state=conv1d_state_bxdxh)

        return x, y, new_cache, recurrent_block_internals

    def _pad_input_for_conv1d(self, x):
        conv1d_cache_input_bxtemporalxh = x[:, 0:1, :].repeat(1,
                                                              self.conv1d_temporal_width - 1,
                                                              1)
        return conv1d_cache_input_bxtemporalxh
    ...
    def forward(
            ...
            cache: RecurrentBlockCache | None = None,
            return_cache: bool = True,
    ) -> tuple[Tensor, None, dict[str, Any]] | tuple[Tensor, RecurrentBlockCache, dict[str, Any]]:
        """Calls the recurrent block.
        ...
        Returns:
          ...
          the input sequence.
        """
        x_bxtxh, y_bxtxh, new_cache, recurrent_block_internals = self.input_to_state(x, segment_pos, cache,
                                                                                     return_cache)

        # Join branches.
        z_bxtxh = self.state_to_output(x_bxtxh, y_bxtxh)
        # output = self.linear_out(z_bxtxh)

        if not return_cache:
            return z_bxtxh, None, recurrent_block_internals

        return z_bxtxh, new_cache, recurrent_block_internals
    ...
    def compute_fixed_point(self, input_bxtxd: torch.Tensor
        ...
        Args:
            ...
            current_state_1xbxh: Optional initial hidden state, defaults to zeros if not provided. shape: 1xbxh
        ...
        input_cache = RecurrentBlockCache(
            rg_lru_state=self.recurrent_block.rg_lru.init_cache(batch_size, self.hidden_size),
            conv1d_state=None)

        # Create a segment_pos tensor which is only ones (no reset)
        segment_pos = torch.ones((batch_size, 1), device=input_bxtxd.device)

        x, y, new_cache, recurrent_block_internals = self.input_to_state(input_bxtxd, segment_pos, input_cache,
                                                                         return_cache=True)
        rg_lru_internals = recurrent_block_internals['rg_lru_internals']

        rg_lru_fixed_point = rg_lru_internals['normalized_x'] / (1 - rg_lru_internals['a'])
        z_fixed_point = rg_lru_fixed_point * y[:, 0:1, :]

        return rg_lru_fixed_point, z_fixed_point
