...
class TransformerDVAEFixedEncoder(nn.Module):
    def __init__(self, config: TinyDVAEConfig):
        super().__init__()
        self.config = config
        self.latent_dim = config.latent_dim
        ...
        self.register_buffer("encoder_std", torch.full([config.latent_dim], 0.1))
        # self.encoder_std = nn.Parameter(torch.ones([self.latent_dim]), requires_grad=True)
    ...
    def inference(self, x, n_layers=None, return_internals=False):
        ...
        # else:
            # enc_out = self.encoder(x, return_internals=return_internals, n_layers=n_layers, use_head=False) # (B, T, E)
        mu = self.encoder(x) # [B, latent_dim]
        std = self.encoder_std.repeat([B, 1]) # [B, latent_dim]
        ...
        # L = self.build_cholesky_L(logvar)
        # dist_q = MultivariateNormal(loc=mu, scale_tril=L)
        dist_q = MultivariateNormal(loc=mu, covariance_matrix=torch.diag_embed(torch.exp(logvar)))
        z = dist_q.rsample()
        # z = self.reparameterize(mu, logvar) # (B, latent_dim)
